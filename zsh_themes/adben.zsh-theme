...
prompt_context () {
    if [[ "$USER" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
		echo -n "${PR_RESET}${PR_RED}☭X3N✇☭ ${PR_RESET}${PR_BRIGHT_YELLOW}%~%<<${PR_RESET}"
    else
...
${GREEN_START_P1}'
    RPROMPT='${PR_RESET}$(git_prompt_info)$(svn_prompt_info)${PR_YELLOW}%D{%R.%S  %b %d %Y} ${GREEN_END}${PR_RESET}'
